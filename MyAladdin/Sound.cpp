...
Sound::Sound()
{
	...
	this->loadSound(SOUND_BOXBELL);
	this->loadSound(SOUND_ABU_WAVING);
	this->loadSound(SOUND_SWORD_SPINNING);
	this->loadSound(SOUND_SWORD_CHING);
	this->loadSound(SOUND_OBJECT_THROW);
	this->loadSound(SOUND_ALADDIN_CONTINUE_POINT);
	this->loadSound(SOUND_REACH_APPLE);
	this->loadSound(SOUND_LEVELCOMPLETE);
	...
	this->loadSound(SOUND_GUARD_A);
	this->loadSound(SOUND_GUARD_E);
	this->loadSound(SOUND_GUARD_O);
	this->loadSound(SOUND_JAFAR_LAUGH);
	this->loadSound(SOUND_JAFAR_SNAKE);
	this->loadSound(SOUND_CAMEL);
	this->loadSound(SOUND_PEDDLER_SHOP);
	...
	this->loadSound(SOUND_TAKE_TEAPOT);
	this->loadSound(SOUND_TAKE_ABU);
	this->loadSound(SOUND_A_DEAL);

	this->setVolume(85, SOUND_SWORD_SPINNING);
...
{
	if (this->_currentBackGround != name)
	{
		this->stop(_currentBackGround);
		this->_currentBackGround = name;
	}
	this->play(_currentBackGround, isLoop, 1, volumne);
}
...
void Sound::loadSound(char * path)
{
	...
	if (error != 0)
	{
		Global::GLPostMessage("can't load ");
		return;
	}
	...
	{
		Global::GLPostMessage("file format does not support ");
		return;
	}
	...
	if (FAILED(result))
	{
		Global::GLPostMessage(" Can not create secondaryBuffers ");
		return;
	}
	...
	if (error != 0)
	{
		Global::GLPostMessage("Can't close file ");
	}
