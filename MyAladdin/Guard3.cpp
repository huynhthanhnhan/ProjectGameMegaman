...
void Guard3::update(float deltaTime)
{
	if (this->GetCurrentFrame(Global::Hurting) == 1 && this->_state == Global::Hurting)
	{
		...
		this->_sound->play(SOUND_GUARD_PAINTS);
	}
...
void Guard3::LoadResource()
{
	...
	guardStand[0]->setCenter(19);
	guardStand[1]->setCenter(16);
	guardStand[2]->setCenter(15);
	guardStand[3]->setCenter(16);
	guardStand[4]->setCenter(21);
	guardStand[5]->setCenter(23);
	guardStand[6]->setCenter(22);
...
void Guard3::UpdateRender(Global::EState currentState)
{
	...
	case Global::Attack:
	{
		if (this->GetCurrentFrame(this->_state) == 2)
		{
			...
			BulletManager::Instance()->addBullet(new Sword(swordLocation.x, swordLocation.y, this->_direct));
			this->_sound = Sound::Instance();
			this->_sound->play(SOUND_SWORD_SPINNING);
		}
		if (this->GetCurrentFrame(this->_state) == 1 && this->_time < 4)
		{
			this->SetCurrentFrame(this->_state, this->GetCurrentFrame(this->_state) - 1);
			this->_time++;
		}
		else this->_time = 0;
		break;
	...
	case Global::Hurting:
	{
		if (this->GetCurrentFrame(this->_state) == 6 && this->_time < 6)
		{
			this->SetCurrentFrame(this->_state, 5);
			this->_time++;
		}
		else this->_time = 0;
		break;
	...
	default:
	{
		if (this->GetCurrentFrame(this->_state) >= 1 && this->_time < 1)
		{
			this->SetCurrentFrame(this->_state, this->GetCurrentFrame(this->_state) - 1);
			this->_time++;
		}
