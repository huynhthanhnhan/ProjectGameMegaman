...
bool Civilian3::isAttack()
{
	...
	return ((this->_state == Global::Attack) && (this->GetCurrentFrame(this->_state) == 3));
...
void Civilian3::UpdateRender(Global::EState currentState)
{
	...
	case Global::Stand:
	{
		if ((this->GetCurrentFrame(_state) == 1 || this->GetCurrentFrame(_state) == 0) && this->_timeAction < this->_maxStand)
		{
			if (this->GetCurrentFrame(_state) == 1)
				this->SetCurrentFrame(_state, 0);
			else
				this->SetCurrentFrame(_state, 5);
			this->_timeAction++;
		}
		else
		{
	...
	case Global::Run:
	{
		...
		{
			int index = (this->GetCurrentFrame(_state) + 1) % this->mListSprite[_state]->GetListRect().size();
			this->SetCurrentFrame(_state, index);
			this->_timeAction = 0;
		}
		else
			this->_timeAction++;
