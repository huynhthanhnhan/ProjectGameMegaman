...
void Guard2::update(float deltaTime)
{
	...
	switch (_state)
	{
	case Global::OnTrap:this->_vx = V_GUARD2/3; break;
...
void Guard2::LoadResource()
{
	...
	guardStand[4]->setCenter(37);
	guardStand[5]->setCenter(32);
	guardStand[6]->setCenter(37);
	guardStand[7]->setCenter(46);
...
void Guard2::UpdateRender(Global::EState currentState)
{
	...
	case Global::Attack:
	{
		...
		if (this->GetCurrentFrame(this->_state) == 10)
		{
			this->_sound = Sound::Instance();
		...
		{
			this->SetCurrentFrame(this->_state, 6);
			this->_time++;
		}
	...
	case Global::Stand:
	{
		if (this->GetCurrentFrame(_state) == 1)
		{
			this->_sound = Sound::Instance();
			this->_sound->play(SOUND_COMEON);
		}
		if (this->GetCurrentFrame(this->_state)>= 2 && this->_timeAction < 1)
		{
		...
		if (this->GetCurrentFrame(this->_state) == 0 && this->_time < 3)
		{
			this->SetCurrentFrame(this->_state, 3);
			this->_time++;
		}
		else if (this->GetCurrentFrame(this->_state) == 1)
			this->_time = 0;
